#!/usr/bin/env bash
# Base/학습 Qwen 모델의 code memorization 정확도·counterfactual·TRACE 평가
set -euo pipefail
cd "$(dirname "$0")"

PYTHON_BIN=${PYTHON_BIN:-/venv/verl/bin/python}
BASE_MODEL=${BASE_MODEL:-Qwen/Qwen2.5-7B-Instruct}
TRAINED_MODEL=${TRAINED_MODEL:-ckpt_hf/Qwen2.5-7B-Instruct/code_memorization_seen}
DATA=${DATA:-data/code/memorization}
OUT=${OUT:-runs/Qwen2.5-7B-Instruct/code_memorization}
...
# 빠른 검증 예: LIMIT_PER_SPLIT=10 ./inference_memorization_code_qwen.sh
